...
namespace version_manager {
    ...
    private:
        ...
        void
        resolve_conflict(std::string const & start_v, std::string const & end_v, json && package, bool from_external);

        std::pair<json, json>
        operational_transform(std::string const & start, json && new_path, json const & conflicting_path,
                              bool from_external);

        std::pair<json, json> ot_on_type(std::string const & tpname, std::string const & start, json && new_tp_change,
                                         json const & conflicting_tp_change, bool from_external);

        std::pair<json, json> ot_on_obj(std::string const & tpname, std::string const & oid, std::string const & start,
                                        json && new_obj_change, json && conflicting_obj_change, bool from_external);

        std::pair<json, json>
        resolve_modification(std::string const & dtpname, json && new_change, json && conflicting_change);
        ...
        std::pair<json, json> run_custom_merge(std::string const & version, std::string const & dtpname,
    ...
    public:
        ...
        VersionManager(std::string const & app_name, have_custom_merge_t && have_merge,
                       custom_merge_function_t && custom_merge_function,
                       type_dim_map_t && type_dim_map,
                       enums::Autoresolve autoresolve = enums::Autoresolve::FullResolve);

        bool receive_data(std::string const & app_name, std::string const & start_v, std::string const & end_v,
                          json && recv_diff, bool from_external = true);

        std::tuple<json, std::string, std::string>
        retrieve_data(std::string const & app_name, std::string version, json && req_types = json());
