...
version_manager::VersionManager::VersionManager(std::string const & app_name, have_custom_merge_t && have_merge,
...
bool version_manager::VersionManager::receive_data(std::string const & app_name, const std::string & start_v,
...
//        if (autoresolve == enums::Autoresolve::BranchExternalPush) {
//            graph.continue_chain(start_v, end_v, json(package), app_name != this->app_name);
//        }
...
json version_manager::VersionManager::filter_diff(json && recv_diff, std::set<std::string> const & types) {
...
void version_manager::VersionManager::data_sent_confirmed(std::string const & app_name, std::string const & start_v,
...
std::set<std::string> version_manager::VersionManager::process_req_types(json && req_types) {
...
version_manager::VersionManager::retrieve_data_nomaintain(std::string version, json && req_types) {
    //std::cout <<"retrieve: " << version << req_types << std::endl;
    std::set<std::string> const & obtainable_types =
            req_types.is_null() ? type_names : process_req_types(std::move(req_types));
    ...
    try {

        {
            ...
            while (graph_it && graph_it != graph_end) {
                ...
                while (delta_it != delta_end && type_it != type_end) {
                    ...
                    if (!compare_result) {
                        package_map.emplace_hint(package_map.end(), *type_it, delta_it->second);
                        ++delta_it;
                        ++type_it;
                        continue;
                    }
                ...
                merged = utils::merge_state_delta(std::move(merged), std::move(package));
        ...
        std::cout << "exception in retrieve_data_nomaintain" << std::endl;
        std::cout << "version : " << version << std::endl;
    }
}

std::tuple<json, std::string, std::string>
version_manager::VersionManager::retrieve_data(const std::string & app_name, std::string version, json && req_types) {
    auto [data, prev_version, next_version] = retrieve_data_nomaintain(version, std::move(req_types));
    graph.mark_app_ref_await(app_name, next_version);
    return {std::move(data), std::move(prev_version), std::move(next_version)};
}

void version_manager::VersionManager::resolve_conflict(const std::string & start_v, const std::string & end_v, json && package,
                                                       bool from_external) {
    auto [change, old_v, new_v] = retrieve_data_nomaintain(start_v, json());

    if (new_v == start_v || autoresolve == enums::Autoresolve::BranchConflicts) {
        graph.continue_chain(start_v, end_v, std::move(package));
        return;
    }
    if (change.is_null()) return;
    auto [new_merge, conflict_merge] = operational_transform(start_v, std::move(change), package, from_external);
...
std::pair<json, json> version_manager::VersionManager::operational_transform(std::string const & start, json && new_path,
                                                                             json const & conflicting_path,
                                                                             bool from_external) {
    ...
    while (new_path_it != new_path_end || conflicting_path_it != conflicting_path_end) {
...
std::pair<json, json> version_manager::VersionManager::ot_on_type(std::string const & tpname, std::string const & start,
                                                                  json && new_tp_change,
                                                                  json const & conflicting_tp_change,
                                                                  bool from_external) {
...
std::pair<json, json> version_manager::VersionManager::ot_on_obj(std::string const & tpname, std::string const & oid,
    ...
    if (eq_in_int(event_new, Event::New) && eq_in_int(event_conf, Event::New)) {
        if (have_custom_merge && have_custom_merge(tpname)) {
            ...
            auto [obj_merge_temp, obj_conf_merge_temp] =
                    from_external ?
                    run_custom_merge(start, tpname, oid, json(), std::move(new_obj_change),
                                     std::move(conflicting_obj_change), new_change_cbor, conflicting_change_cbor) :
                    run_custom_merge(start, tpname, oid, json(), std::move(conflicting_obj_change),
            ...
            obj_conf_merge = dim_diff(tpname, std::move(conflicting_obj_change), std::move(new_obj_change));
        }
