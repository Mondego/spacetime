...
def updater(df, obj_ids, count_upto, index):
    stop = Stop(index)
    ...
    while not stop.start:
        df.pull_await()
    count_objs = [df.read_one(BasicCounter, oid) for oid in obj_ids]
    lost_time = 0
    while min(o.count for o in count_objs) < count_upto:
...
def run_bench(obj_count, num_consumers, rn):
    ...
    for i, upd_node in enumerate(updaters):
        upd_node.start_async(list(range(i, obj_count, num_consumers)), count_upto, i)
