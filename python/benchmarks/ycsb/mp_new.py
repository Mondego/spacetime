...
def reader(host, port, reader_id, obj_count, event, rret, read_event):
    ...
    rret.put((0.0, sum(final_ts.values()) / obj_count, op, et - st))
    #return 0.0, op, et - st
...
class State(object):
    ...
    def add_client(self, client_id, client_con):
    ...
    def write(self, data):
        for cid, con in self.clients.items():
            with self.client_locks[cid].gen_wlock():
                if not send_all(con, data):
                    raise RuntimeError("Broken server client connection")
...
def receive_updates(con, state):
    resp = receive_data(con, "from accept")
    if resp is None:
        print ("Broken accept connection")
        return
    cid, tp = resp
    if tp != "WRITER":
        state.add_client(cid, con)
        return
    while True:
...
def run_expt(args, num_prod):
    ...
    if args.run_server:
        ...
        resp = requests.get(f"http://{args.host}:10000/mp_start")
        if not resp:
    ...
    if args.run_server:
        ...
        resp = requests.get(f"http://{args.host}:10000/mp_stop")
        if not resp:
