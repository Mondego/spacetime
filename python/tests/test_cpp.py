...
def server_df1(send_q, recv_q, server_ready, client_ready):
    df = Dataframe("SERVER_TEST1", [Car], is_server=True)
    ...
    assert (c3.xvel == c4.xvel)
    assert (c3.yvel == c4.yvel)
    assert (c3.xpos == c4.xpos)
    assert (c3.ypos == c4.ypos)
    assert (c3.oid == c4.oid)
    c2.yvel = 1
    c2.xvel = 1
    df.delete_one(Car, c2)
    assert (df.read_one(Car, 1) == None)
    assert (c2.__r_df__ == None)
    assert (c2.xvel == 1)
    assert (c2.yvel == 1)
    c2.xvel = 2
    c2.yvel = 2
...
def client_df1(send_q, recv_q, server_ready, client_ready):
    ...
    assert (df.read_one(Car, 1) == None)
    assert (df.read_one(Car, 2) == None)
    # Th== does not work yet. Have to figure it out.
...
def server_df2(send_q, recv_q, server_ready, client_ready):
    df = Dataframe("SERVER_TEST2", [Car], is_server=True)
    send_q.put(df.details)
    client_name = recv_q.get()
    # The server goes first.
...
def server_df3(send_q, recv_q, server_ready, client_ready):
    df = Dataframe("SERVER_TEST3", [Counter], resolver={Counter: counter_merge_func}, is_server=True)
    send_q.put(df.details)
    client_name = recv_q.get()
    # The server goes first.
    #print ("Server at start:", df.versioned_heap.version_graph.nodes.keys())
...
def server_df4(send_q, recv_q, server_ready, client_ready):
    df = Dataframe("SERVER_TEST4", [Car, Counter], resolver={Counter: counter_merge_func}, is_server=True)
    send_q.put(df.details)
    client_name = recv_q.get()
    # The server goes first.
    #print ("Server at start:", df.versioned_heap.version_graph.nodes.keys())
...
def server_df5(send_q, recv_q, server_ready, client_ready):
    df = Dataframe("SERVER_TEST5", [Car], is_server=True)
    send_q.put(df.details)
    client_name = recv_q.get()
    # The server goes first.
...
def server_df6(send_q, recv_q, server_ready, client_ready):
    df = Dataframe("SERVER_TEST6", [ClassWithCounter, Counter], resolver={Counter: counter_merge_func}, is_server=True)
    send_q.put(df.details)
    client_name = recv_q.get()
    # The server goes first.
...
def server_df7(send_q, recv_q, server_ready, client_ready):
    df = Dataframe("SERVER_TEST7", [Blocker], resolver={Blocker: blocker_merge_func}, is_server=True)
    send_q.put(df.details)
    client_name = recv_q.get()
    # The server goes first.
    #print ("Server at start:", df.versioned_heap.version_graph.nodes.keys())
