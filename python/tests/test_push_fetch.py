    ...
    def test_base_set(self):
        ...
        self.assertDictEqual(
            Car.__r_table__.object_table, {0: {
                "oid": 0, "xvel": 0, "yvel": 0, "xpos": 0, "ypos": 0
            }})
        c.xvel = 1
        self.assertDictEqual(
            Car.__r_table__.object_table, {0: {
                "oid": 0, "xvel": 1, "yvel": 0, "xpos": 0, "ypos": 0
            }})
        c.oid = 1
        self.assertDictEqual(
            Car.__r_table__.object_table, {1: {
                "oid": 1, "xvel": 1, "yvel": 0, "xpos": 0, "ypos": 0
            }})
...
class TestFullStateDataframeBasic(unittest.TestCase):
    def test_basic_delete2(self):
        ...
        self.assertEqual(df.heap, c.__r_df__)
        ...
        df.commit()
        df.version_graph.put("TEST_ALT", {"TEST_ALT": "NEXT"}, [(
            head, "NEXT", {
                Car.__r_meta__.name: {
                    0: {
                        "types": {
...
"""
    def test_push1(self):
        ...
        df2 = Dataframe("TEST2", [Car], remotes={"TEST1": df1.details})
        ...
        self.assertDictEqual(
            df1.versioned_heap.state_to_app,
            {"ROOT": set([appname2]), version1: set([appname2])})
        self.assertDictEqual(
...
def client_df1(send_q, recv_q, server_ready, client_ready):
    ...
    df = Dataframe("CLIENT_TEST", [Car], remotes={"SERVER_TEST1": server_name})
    send_q.put(df.details)
    # Waiting at point C1
    server_ready.wait()
    server_ready.clear()
...
def client_df2(send_q, recv_q, server_ready, client_ready):
    server_name = recv_q.get()
    df = Dataframe("CLIENT_TEST", [Car], remotes={"SERVER_TEST2": server_name})
    send_q.put(df.details)
...
def client_df3(send_q, recv_q, server_ready, client_ready):
    ...
    df = Dataframe(
        "CLIENT_TEST",
        [Counter],
        remotes={"SERVER_TEST3": server_name},
...
def client_df4(send_q, recv_q, server_ready, client_ready):
    ...
    df = Dataframe("CLIENT_TEST", [Counter],
                   remotes={"SERVER_TEST4": server_name},
                   resolver={Counter: counter_merge_func})
    send_q.put(df.details)
    #print ("Client at start:", df.versioned_heap.version_graph.nodes.keys())
    # Waiting at point C1
...
def client_df5(send_q, recv_q, server_ready, client_ready):
    server_name = recv_q.get()
    df = Dataframe("CLIENT_TEST", [Car], remotes={"SERVER_TEST5": server_name})
    send_q.put(df.details)
    # Waiting at point C1
    server_ready.wait()
    server_ready.clear()
...
def client_df6(send_q, recv_q, server_ready, client_ready):
    server_name = recv_q.get()
    df = Dataframe(
        "CLIENT_TEST", [ClassWithCounter, Counter],
        remotes={"SERVER_TEST6": server_name},
        resolver={Counter: counter_merge_func})
    send_q.put(df.details)
    # Waiting at point C1
    server_ready.wait()
...
def client_df7(send_q, recv_q, server_ready, client_ready):
    ...
    df = Dataframe(
        "CLIENT_TEST", [Blocker],
        remotes={"SERVER_TEST7": server_name},
